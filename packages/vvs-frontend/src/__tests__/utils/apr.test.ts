...
describe('getFarmApr', () => {
  it(`returns null when parameters are missing`, () => {
    const { vvsRewardsApr, lpRewardsApr } = getFarmApr(null, null, null, null)
    expect(vvsRewardsApr).toBeNull()
    expect(lpRewardsApr).toEqual(0)
  })
  it(`returns null when APR is infinite`, () => {
    const { vvsRewardsApr, lpRewardsApr } = getFarmApr(BIG_ZERO, BIG_ZERO, BIG_ZERO, '')
    expect(vvsRewardsApr).toBeNull()
    expect(lpRewardsApr).toEqual(0)
  })
  it(`get the correct pool APR`, () => {
    const { vvsRewardsApr, lpRewardsApr } = getFarmApr(BIG_TEN, new BigNumber(1), new BigNumber(100000), '')
    expect(vvsRewardsApr).toEqual(4204800)
    expect(lpRewardsApr).toEqual(0)
  })
