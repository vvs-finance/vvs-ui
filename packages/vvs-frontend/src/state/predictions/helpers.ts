...
export const transformBetResponse = (betResponse: BetResponse): Bet => {
  const bet = {
    ...
    claimedCRO: betResponse.claimedCRO ? parseFloat(betResponse.claimedCRO) : 0,
    claimedNetCRO: betResponse.claimedNetCRO ? parseFloat(betResponse.claimedNetCRO) : 0,
...
export const transformUserResponse = (userResponse: UserResponse): PredictionUser => {
  ...
  return {
    ...
    totalCRO: totalCRO ? parseFloat(totalCRO) : 0,
    totalCROBull: totalCROBull ? parseFloat(totalCROBull) : 0,
    totalCROBear: totalCROBear ? parseFloat(totalCROBear) : 0,
    totalBetsClaimed: numberOrNull(totalBetsClaimed),
    totalCROClaimed: totalCROClaimed ? parseFloat(totalCROClaimed) : 0,
    winRate: winRate ? parseFloat(winRate) : 0,
    averageCRO: averageCRO ? parseFloat(averageCRO) : 0,
    netCRO: netCRO ? parseFloat(netCRO) : 0,
  }
}
...
export const getTotalWon = async (): Promise<number> => {
  ...
  const totalCRO = market.totalCRO ? parseFloat(market.totalCRO) : 0
  const totalCROTreasury = market.totalCROTreasury ? parseFloat(market.totalCROTreasury) : 0

  return Math.max(totalCRO - totalCROTreasury, 0)
}
