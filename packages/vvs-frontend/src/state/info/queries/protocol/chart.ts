...
import { VVS_FINANCE_START } from 'config/constants/info'
import { ChartEntry } from 'state/info/types'
import { VvsDayDatasResponse } from '../types'
import { fetchChartData, mapDayData } from '../helpers'

/**
 * Data for displaying Liquidity and Volume charts on Overview page
 */
const VVS_DAY_DATAS = gql`
  query overviewCharts($startTime: Int!, $skip: Int!) {
    vvsDayDatas(first: 1000, skip: $skip, where: { date_gt: $startTime }, orderBy: date, orderDirection: asc) {
      date
      dailyVolumeUSD
      totalLiquidityUSD
    }
...
const getOverviewChartData = async (skip: number): Promise<{ data?: ChartEntry[]; error: boolean }> => {
  try {
    const { vvsDayDatas } = await request<VvsDayDatasResponse>(INFO_CLIENT, VVS_DAY_DATAS, {
      startTime: VVS_FINANCE_START,
      skip,
    })
    const data = vvsDayDatas.map(mapDayData)
