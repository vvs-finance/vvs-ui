...
- `chainId` - what chain ID to use, if not provided defaults to `28`
...
It also sets `profile_${address}` cookie containing username and avatar (now only for vvs.finance domain, maybe configurable in future versions)

```js
import PancakeProfileSdk from "@vvs-swap/profile-sdk";
